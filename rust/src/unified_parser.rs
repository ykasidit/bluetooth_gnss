...
use crate::{NMEA_PARSER, OUTPUT_STATE_PARAMS_MAP};

lazy_static! {
    pub static ref QSTARZ_BLE_BUFFER: Mutex<Vec<Vec<u8>>> = Mutex::new(Vec::new());
}

pub fn clear_qstarz_ble_buffer() {
    QSTARZ_BLE_BUFFER.lock().unwrap().clear();
}

/// Unified entry point: accepts raw bytes + protocol hint, returns JSON array string.
pub fn unified_feed_and_parse(data: &[u8], hint: ProtocolHint) -> String {
    match hint {
        ProtocolHint::AutoDetectStream => feed_nmea_ubx_stream(data),
        ProtocolHint::QstarzBleChunk => feed_qstarz_ble_chunk(data),
    }
}

/// Feed NMEA/UBX bytes into the existing queue_and_parse pipeline.
fn feed_nmea_ubx_stream(data: &[u8]) -> String {
    ...
    let parsed_result = queue_and_parse(&mut param_state, nmea_parser_state, data);
    match parsed_result {
...
fn feed_qstarz_ble_chunk(data: &[u8]) -> String {
    let mut buf = QSTARZ_BLE_BUFFER.lock().unwrap();
...
mod tests {
    ...
    fn test_feed_nmea_stream() {
        ...
        let result = unified_feed_and_parse(nmea, ProtocolHint::AutoDetectStream);
    ...
    fn test_feed_qstarz_4_chunk_gps_not_fixed() {
        ...
        assert_eq!(unified_feed_and_parse(&chunk1, ProtocolHint::QstarzBleChunk), "[]");
        assert_eq!(unified_feed_and_parse(&chunk2, ProtocolHint::QstarzBleChunk), "[]");
        assert_eq!(unified_feed_and_parse(&chunk3, ProtocolHint::QstarzBleChunk), "[]");

        // Fourth chunk triggers parse
        let r4 = unified_feed_and_parse(&chunk4, ProtocolHint::QstarzBleChunk);
    ...
    fn test_feed_qstarz_3_chunk_gps_fixed_wo_gsv() {
        ...
        let r4 = unified_feed_and_parse(&dummy_chunk, ProtocolHint::QstarzBleChunk);
        let arr: Vec<serde_json::Value> = serde_json::from_str(&r4).unwrap();
        assert_eq!(arr.len(), 1);
    ...
    fn test_feed_qstarz_consecutive_packets() {
        ...
        for i in 0..3 {
            assert_eq!(unified_feed_and_parse(&p1_chunks[i], ProtocolHint::QstarzBleChunk), "[]");
        }
        let r1 = unified_feed_and_parse(&p1_chunks[3], ProtocolHint::QstarzBleChunk);
        ...
        for i in 0..3 {
            assert_eq!(unified_feed_and_parse(&p2_chunks[i], ProtocolHint::QstarzBleChunk), "[]");
        }
        let r2 = unified_feed_and_parse(&p2_chunks[3], ProtocolHint::QstarzBleChunk);
    ...
    fn test_feed_qstarz_invalid_first_byte_ignored() {
        ...
        assert_eq!(unified_feed_and_parse(&bad_chunk1, ProtocolHint::QstarzBleChunk), "[]");
        assert_eq!(unified_feed_and_parse(&chunk2, ProtocolHint::QstarzBleChunk), "[]");
        assert_eq!(unified_feed_and_parse(&chunk3, ProtocolHint::QstarzBleChunk), "[]");
        // Should still be [] because first chunk has invalid fix_status
        assert_eq!(unified_feed_and_parse(&chunk4, ProtocolHint::QstarzBleChunk), "[]");
    }
    ...
    fn test_feed_nmea_partial_then_complete() {
        ...
        let r1 = unified_feed_and_parse(part1, ProtocolHint::AutoDetectStream);
        let arr1: Vec<serde_json::Value> = serde_json::from_str(&r1).unwrap();
        assert_eq!(arr1.len(), 0); // not enough data yet

        let r2 = unified_feed_and_parse(part2, ProtocolHint::AutoDetectStream);
        let arr2: Vec<serde_json::Value> = serde_json::from_str(&r2).unwrap();
    ...
    fn test_feed_nmea_multiple_sentences_in_one_call() {
        ...
        let result = unified_feed_and_parse(multi, ProtocolHint::AutoDetectStream);
    ...
    fn test_feed_nmea_with_garbage_prefix() {
        ...
        let result = unified_feed_and_parse(&data, ProtocolHint::AutoDetectStream);
        let arr: Vec<serde_json::Value> = serde_json::from_str(&result).unwrap();
        assert_eq!(arr.len(), 1);
        assert_eq!(arr[0]["name"], "GSV");
    }
    ...
    fn test_feed_nmea_crlf_and_lf() {
        ...
        let result = unified_feed_and_parse(crlf, ProtocolHint::AutoDetectStream);
        let arr: Vec<serde_json::Value> = serde_json::from_str(&result).unwrap();
        assert_eq!(arr.len(), 1);
        assert_eq!(arr[0]["name"], "GSV");
    }
